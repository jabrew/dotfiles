; From https://superuser.com/questions/770014/is-it-possible-to-retain-multi-monitor-settings-when-one-monitor-is-unplugged/825764#825764

; Changes: Replace %TEMP% with %WindowPositionLocation%
WindowPositionLocation := "C:\Users\Brew\Documents\Autohotkey\WinPositions"

FileDelete, %WindowPositionLocation%\WinPosWorking.txt
FileCopy, %WindowPositionLocation%\WinPos.txt, %WindowPositionLocation%\WinPosWorking.txt

FileGetTime, FechaOriginal, %WindowPositionLocation%\WinPos.txt
...
{
    Loop, Read, %WindowPositionLocation%\WinPosWorking.txt   ; This loop retrieves each line from the file, one at a time.
    {
        ArrayCount += 1  ; Keep track of how many items are in the array.
        Array%ArrayCount% := A_LoopReadLine  ; Store this line in the next array element.
    }
    FileAppend, % "# " . A_YYYY . "-" . A_MM . "-" . A_DD . " " . A_Hour . ":" . A_Min . ":" . A_Sec . " --> " . "RestoreWinPositions:--> Fichero de posiciones creado hace " . TiempoTranscurrido . " segundos (Correcto). Restaurando ventanas...." . " `n", %WindowPositionLocation%\WinPositions-Log.txt
...
{
    FileAppend, % "# " . A_YYYY . "-" . A_MM . "-" . A_DD . " " . A_Hour . ":" . A_Min . ":" . A_Sec . " --> " . "RestoreWinPositions:--> Fichero de posiciones creado hace " . TiempoTranscurrido . " segundos (demasiado reciente): Salvapantallas activado en falso. Abortando..." . " `n", %WindowPositionLocation%\WinPositions-Log.txt
...
{       ; Si no hay parámetros, capturamos el número de pantallas del fichero de configuración (por defecto en el directorio de ejecución del programa), si existe:
    ...
    {   ; ... leemos de él el número de pantallas esperado.
        ...
        FileAppend, % "# " . A_YYYY . "-" . A_MM . "-" . A_DD . " " . A_Hour . ":" . A_Min . ":" . A_Sec . " --> " . "RestoreWinPositions:--> No se introdujeron parámetros, se utilizará el extraído del fichero de configuración.`n", %WindowPositionLocation%\WinPositions-Log.txt
    } else
    {
        FileAppend, % "# " . A_YYYY . "-" . A_MM . "-" . A_DD . " " . A_Hour . ":" . A_Min . ":" . A_Sec . " --> " . "RestoreWinPositions:--> No existe el fichero de configuración.`n", %WindowPositionLocation%\WinPositions-Log.txt
    }
} else
{   ; Si hay parámetros...
    ...
    FileAppend, % "# " . A_YYYY . "-" . A_MM . "-" . A_DD . " " . A_Hour . ":" . A_Min . ":" . A_Sec . " --> " . "RestoreWinPositions:-> Ignorando número de monitores: se ejecutará el programa sin esperarlos.`n", %WindowPositionLocation%\WinPositions-Log.txt
    } else
...
{   ; ... añadimos una entrada al log con el número de pantallas necesarias.
    FileAppend, % "# " . A_YYYY . "-" . A_MM . "-" . A_DD . " " . A_Hour . ":" . A_Min . ":" . A_Sec . " --> " . "RestoreWinPositions:--> El sistema debe tener al menos " . NumeroDePantallasEsperado " pantalla(s).`n", %WindowPositionLocation%\WinPositions-Log.txt
}
...
{   ; ... esperamos a que se enciendan.
    FileAppend, % "# " . A_YYYY . "-" . A_MM . "-" . A_DD . " " . A_Hour . ":" . A_Min . ":" . A_Sec . " --> " . "RestoreWinPositions:-> Esperando a que el sistema tenga al menos " . NumeroDePantallasEsperado " pantalla(s).`n", %WindowPositionLocation%\WinPositions-Log.txt
    InicioEspera := A_Now
...
While ( not IgnoreNumPantallas and m < NumeroDePantallasEsperado )
{
    ...
    {   ; Superado tiempo de espera por pantallas.
        FileAppend, % "# " . A_YYYY . "-" . A_MM . "-" . A_DD . " " . A_Hour . ":" . A_Min . ":" . A_Sec . " --> " . "RestoreWinPositions:-> No están listas las " . NumeroDePantallasEsperado . " pantallas transcurrido el tiempo límite. Cancelando la ejecución del programa.`n", %WindowPositionLocation%\WinPositions-Log.txt
...
While IncorrectPositionWindows>0
{
    If Checking>0
    {
        FileAppend, % "# " . A_YYYY . "-" . A_MM . "-" . A_DD . " " . A_Hour . ":" . A_Min . ":" . A_Sec . " --> " . "RestoreWinPositions:--> Alguna(s) ventana(s) no estaban en su posición correcta. Revisando; pasada número " . Checking+1 . " ...." . " `n", %WindowPositionLocation%\WinPositions-Log.txt
    }
    ...
    {       ; Extraemos línea a línea los datos de cada ventana individual
        ...
        {   IfWinExist, ahk_id %ID% ; Actuaremos sobre la ventana tan solo si la ventana existe.
            {   ; Incrementamos el contador de ventanas que no están en su posición correcta (es necesario moverla).
                ...
                FileAppend, % "# " . A_YYYY . "-" . A_MM . "-" . A_DD . " " . A_Hour . ":" . A_Min . ":" . A_Sec . " --> " . "RestoreWinPositions:-->  Moviendo ventana " . Titulo . " de coordenadas (X,Y,W,H): " . actualx . "," . actualy . "," . actualw . "," . actualh . " a coordenadas (X,Y,W,H): " . x . "," . y . "," . w . "," . h . "." . " `n", %WindowPositionLocation%\WinPositions-Log.txt
                If WinStatus = -1   ; Si la ventana está minimizada...
            ...
            {   ; prescindimos de actuar sobre esta ventana.
                FileAppend, % "# " . A_YYYY . "-" . A_MM . "-" . A_DD . " " . A_Hour . ":" . A_Min . ":" . A_Sec . " --> " . "RestoreWinPositions:-->  Omitiendo ventana inexistente: " . Titulo . ". `n", %WindowPositionLocation%\WinPositions-Log.txt
            }
        }
    }
    If Checking>=10 ; ¿Llevamos más de 10 pasadas intentando poner las ventanas en su sitio?
    {   ; Cancelamos
        FileAppend, % "# " . A_YYYY . "-" . A_MM . "-" . A_DD . " " . A_Hour . ":" . A_Min . ":" . A_Sec . " --> " . "RestoreWinPositions:--> Demasiados intentos de ubicar ventanas. Abortando. :-( `n", %WindowPositionLocation%\WinPositions-Log.txt
        TrayTip, RestoreWinPositions, Demasiados intentos de ubicar ventanas. Abortando. :-( , ,1
        sleep 10000
        Exit 2
    }
    FileAppend, % "# " . A_YYYY . "-" . A_MM . "-" . A_DD . " " . A_Hour . ":" . A_Min . ":" . A_Sec . " --> " . "RestoreWinPositions:-->  Ventanas que ya estaban en su posición correcta: " . CorrectPositionWindows . ". `n", %WindowPositionLocation%\WinPositions-Log.txt
    FileAppend, % "# " . A_YYYY . "-" . A_MM . "-" . A_DD . " " . A_Hour . ":" . A_Min . ":" . A_Sec . " --> " . "RestoreWinPositions:-->  Ventanas reubicadas: " . IncorrectPositionWindows . ". `n", %WindowPositionLocation%\WinPositions-Log.txt
    Checking := ++Checking
}
FileAppend, % "# " . A_YYYY . "-" . A_MM . "-" . A_DD . " " . A_Hour . ":" . A_Min . ":" . A_Sec . " --> " . "RestoreWinPositions:--> Ventanas reubicadas corréctamente. Saliendo del programa. :-) `n", %WindowPositionLocation%\WinPositions-Log.txt
